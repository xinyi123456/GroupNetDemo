...
namespace GroupNetDemo
{
    class VzClientSDK
    {
        ...
        [StructLayout(LayoutKind.Sequential)]
        public struct IVS_DG_DEVICE_INFO
        {
            ...
            public char[] vzName;//识别结果来源设备名称

            [System.Runtime.InteropServices.MarshalAsAttribute(System.Runtime.InteropServices.UnmanagedType.ByValArray, SizeConst = 16, ArraySubType = System.Runtime.InteropServices.UnmanagedType.I1)]
            public char[] vzIP; //识别结果来源设备IP

            [System.Runtime.InteropServices.MarshalAsAttribute(System.Runtime.InteropServices.UnmanagedType.ByValArray, SizeConst = 24)]
            public char[] vzSN;//识别结果来源设备序列号

            public int vzType; //组网识别结果设备类型，0 : 入口 1 ： 出口   
        }
...
//        public static extern int VzLPRClient_GetSnapShootToJpeg2(int nPlayHandle, string pFullPathName, int nQuality);
        ...
        */
        [DllImport("VzLPRSDK.dll")]
        public static extern int VzLPRClient_SetDGResultEnable(int handle, bool bEnable);
        ...
        *  @param  [IN] handle			由VzLPRClient_Open函数获得的句柄
        *  @param  [IN] pUserData		回调函数的上下文
        *  @param  [OUT] exitEntryInfo	车辆出入口信息 0 为入口，1 为出口
        *  @param  [OUT] exitIvsInfo		车辆出口信息（当设备为入口设备时该字段信息无效）
        *  @param  [OUT] entryIvsInfo	车辆入口信息
        *  @param  [OUT] exitDGInfo    出口设备信息
        *  @param  [OUT] entryDGInfo   入口设备信息
        *  @return 0表示成功，-1表示失败
        *  @ingroup group_callback
        */
        public delegate int  VZLPRC_PLATE_GROUP_INFO_CALLBACK(int handle, IntPtr pUserData,
                                                              int exitEntryInfo, IntPtr exitIvsInfo, IntPtr entryIvsInfo, IntPtr exitDGInfo, IntPtr entryDGInfo);
        ...
        */
        [DllImport("VzLPRSDK.dll", CallingConvention = CallingConvention.StdCall)]
        public static extern int VzLPRClient_SetPlateGroupInfoCallBack(int handle, VZLPRC_PLATE_GROUP_INFO_CALLBACK func, IntPtr pUserData);
        ...
        */
        [DllImport("VzLPRSDK.dll")]
        public static extern int VzLPRClient_LoadGroupFullImageById(int handle, int id, string sn, IntPtr pdata, ref int size);
        ...
        */
        [DllImport("VzLPRSDK.dll")]
        public static extern int VzLPRClient_LoadGroupClipImageById(int handle, int id, string sn, IntPtr pdata, ref int size);
    }
}
